...
# ただし，DataFrameには行番号・列番号の他に，行ラベル・列ラベルが存在するため，以下のようにこれらを区別する：
# 
# - 行・列番号によって参照する方法：`iloc`属性（NumPyのインデックス参照と同じ）
# - 行・列ラベルによって参照する方法：`loc`属性，角括弧`[]`
...
# **行・列番号による参照：`iloc`属性**
# 
# DataFrameを行番号と列番号によって参照するには以下のように`iloc`属性を用いる：
# 
# ```python
# df.iloc[行番号, 列番号]
# ```
...
# DataFrameにおけるブールインデックス参照は基本的にはNumPyと同様であるが，必ず`loc`属性を用いる．
...
# `loc`属性を用いた参照において，ブールインデックスを指定すると，行ラベルが`True`の行だけを抽出することができる：
# ```python
...
#     - `df`を`BMI`列で昇順に並び替え，先頭から10行を表示せよ．
...
# 次のcsvファイルをダウンロードし，カレントディレクトリに保存せよ：[game.csv](https://drive.google.com/uc?export=download&id=1gueZANYM2wOkQefKpoA_LplKkG0aXA4A)  <br>
# このファイルには，ヨーロッパリーグ（イングランド，フランス，ドイツ，イタリア，スペイン）の2017年シーズンの全試合の結果が記録されている．

# - `game.csv`ファイルを`df`に読み込め
...
# - １つのリーグを選び，このリーグの節数を求めよ．（節番号は`section`列に記録されている．）
...
# - 新たに`total_score`列を作成し，`away_score`列と`home_score`列の合計を計算せよ．
